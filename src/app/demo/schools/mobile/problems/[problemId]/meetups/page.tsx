...
import { getDemoMeetups, type DemoContext } from "@/lib/demoContext";

const CONTEXT: DemoContext = 'schools';

export default function SchoolsMeetupsPage() {
  ...
  const meetups = demoMeetups.filter(meetup => meetup.problemIds.includes(problemId));
  ...
  return (
    <div className="min-h-screen bg-[#141414] text-white">
      {/* Shared Header */}
      <SharedHeader mode="back" backHref={`/demo/schools/mobile/problems/${problemId}`} />
      
      {/* Navigation */}
      <div className="relative z-10 px-3 pt-20 pb-2">
        <ProblemPillNav problemId={problemId} active="meet" context="schools" />
      </div>
        ...
        {meetups.length === 0 && (
          <div className="text-center py-12">
            <p className="text-[16px] leading-[1.3] font-sans text-[rgba(255,255,255,0.6)] mb-4">
              Nie zaplanowano jeszcze spotkań
            </p>
            <p className="text-[14px] leading-[1.3] font-sans text-[rgba(255,255,255,0.4)]">
              Bądź pierwszy i zorganizuj spotkanie w tej sprawie!
            </p>
          </div>
        )}
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 px-3 pb-4">
        <Link
          href={`/demo/schools/mobile/problems/${problemId}/meetups/add`}
