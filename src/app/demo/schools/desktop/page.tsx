...
import { ArrowLeft, BarChart3, Users, Lightbulb, Calendar, TrendingUp, AlertCircle, MapPin, GraduationCap, Clock, Vote } from "lucide-react";
...
const SchoolMap = dynamic(() => import("@/components/SchoolMap"), { 
  ...
  loading: () => <div className="aspect-[16/9] bg-[#1a1a1a] rounded-xl border border-white/10 flex items-center justify-center">
    <div className="text-white/60">Ładowanie mapy...</div>
  </div>
});

const CONTEXT: DemoContext = 'schools';

export default function SchoolsDesktopDashboard() {
  ...
  const stats = {
    ...
    activeStudents: demoLeaderboard.length,
    engagementRate: 74,
    solutionImplementation: 52
  };
      ...
      <header className="border-b border-white/10 bg-black/50 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-medium">Ignis for Schools</h1>
              <p className="text-sm text-white/60">Panel Administracyjny</p>
            </div>
          </div>
          <Link 
            href="/demo/schools"
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        >
          <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-sm text-white/60">Aktywne Problemy</span>
            </div>
            <div className="text-2xl font-semibold">{stats.totalProblems}</div>
            <div className="text-xs text-green-400 mt-1">+6% ten miesiąc</div>
          </div>

          <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
            <div className="flex items-center gap-3 mb-2">
              <Lightbulb size={20} className="text-yellow-400" />
              <span className="text-sm text-white/60">Rozwiązania</span>
            </div>
            <div className="text-2xl font-semibold">{stats.totalIdeas}</div>
            <div className="text-xs text-green-400 mt-1">+14% ten miesiąc</div>
          </div>

          <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
            <div className="flex items-center gap-3 mb-2">
              <Calendar size={20} className="text-blue-400" />
              <span className="text-sm text-white/60">Spotkania</span>
            </div>
            <div className="text-2xl font-semibold">{stats.totalMeetups}</div>
            <div className="text-xs text-green-400 mt-1">+3% ten miesiąc</div>
          </div>

          <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
            <div className="flex items-center gap-3 mb-2">
              <GraduationCap size={20} className="text-purple-400" />
              <span className="text-sm text-white/60">Aktywni Uczniowie</span>
            </div>
            <div className="text-2xl font-semibold">{stats.activeStudents}</div>
            <div className="text-xs text-green-400 mt-1">+11% ten miesiąc</div>
          </div>

          <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
            <div className="flex items-center gap-3 mb-2">
              <TrendingUp size={20} className="text-green-400" />
              <span className="text-sm text-white/60">Zaangażowanie</span>
            </div>
            <div className="text-2xl font-semibold">{stats.engagementRate}%</div>
            <div className="text-xs text-green-400 mt-1">+9% ten miesiąc</div>
          </div>

          <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
            <div className="flex items-center gap-3 mb-2">
              <BarChart3 size={20} className="text-cyan-400" />
              <span className="text-sm text-white/60">Wdrożone</span>
            </div>
            <div className="text-2xl font-semibold">{stats.solutionImplementation}%</div>
            <div className="text-xs text-green-400 mt-1">+16% ten miesiąc</div>
          </div>
        </div>
          ...
          <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
            ...
            <div className="mt-4 flex flex-wrap gap-4 text-xs">
              <div className="flex items-center gap-2">
                ...
                <span className="text-white/60">Pilne (2)</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-orange-400 rounded-full"></div>
                <span className="text-white/60">Średnie (3)</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-yellow-400 rounded-full"></div>
                <span className="text-white/60">Niskie (3)</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-green-400 rounded-full"></div>
                <span className="text-white/60">Rozwiązane (1)</span>
              </div>
            </div>
          </div>
        </div>
        ...
        >
          <h3 className="text-xl font-medium mb-6 flex items-center gap-2">
            ...
            Powiązania Problem-Rozwiązanie-Spotkanie
          </h3>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <div className={`px-2 py-1 rounded-full text-xs font-medium ${
                            ...
                            {problemIndex < 2 ? 'Pilny priorytet' : problemIndex < 3 ? 'Średni priorytet' : 'Niski priorytet'}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-4 text-xs text-white/50">
                          <span>{relatedIdeas.length} zaproponowanych rozwiązań</span>
                          <span>{relatedMeetups.length} zaplanowanych spotkań</span>
                          <span>Utworzono {problemIndex === 0 ? '2h temu' : problemIndex === 1 ? '4h temu' : problemIndex === 2 ? '1 dzień temu' : '2 dni temu'}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-0">
                    ...
                    <div className="p-6 border-r border-white/10">
                      <h5 className="text-sm font-medium text-yellow-400 mb-4 flex items-center gap-2">
                        <Lightbulb size={16} />
                        Proponowane rozwiązania ({relatedIdeas.length})
                      </h5>
                      {relatedIdeas.length > 0 ? (
                              ...
                              <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-xs text-green-400 font-medium">{idea.votesCount} głosów</span>
                                  </div>
                                </div>
                              </div>
                            </div>
                          ...
                          {relatedIdeas.length > 3 && (
                            <div className="text-center">
                              <button className="text-xs text-white/40 hover:text-white/60 transition-colors">
                                +{relatedIdeas.length - 3} więcej rozwiązań
                              </button>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="text-center py-6 text-white/40">
                          ...
                          <p className="text-sm">Nie zaproponowano jeszcze rozwiązań</p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="p-6">
                      <h5 className="text-sm font-medium text-blue-400 mb-4 flex items-center gap-2">
                        <Calendar size={16} />
                        Powiązane spotkania ({relatedMeetups.length})
                      </h5>
                      {relatedMeetups.length > 0 ? (
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-3 text-xs text-white/60 mb-1">
                                    <div className="flex items-center gap-1">
                                      <Clock size={10} />
                                      <span>{meetup.date} o {meetup.time}</span>
                                    </div>
                                  </div>
                                  <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2 text-xs">
                                      <Users size={10} className="text-purple-400" />
                                      <span className="text-purple-400">{meetup.participants} osób</span>
                                    </div>
                        ...
                        <div className="text-center py-6 text-white/40">
                          ...
                          <p className="text-sm">Nie zaplanowano jeszcze spotkań</p>
                        </div>
                      )}
                    </div>
                  </div>
