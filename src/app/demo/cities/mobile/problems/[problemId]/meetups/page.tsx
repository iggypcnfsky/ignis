...
export default function CitiesMeetupsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#141414] text-white">
      {/* Shared Header */}
      <SharedHeader mode="back" backHref={`/demo/cities/mobile/problems/${problemId}`} />
      
      {/* Navigation */}
      <div className="relative z-10 px-3 pt-20 pb-2">
        <ProblemPillNav problemId={problemId} active="meet" context="cities" />
      </div>
        ...
        {meetups.length === 0 && (
          <div className="text-center py-12">
            <p className="text-[16px] leading-[1.3] font-sans text-[rgba(255,255,255,0.6)] mb-4">
              No town halls scheduled yet
            </p>
            <p className="text-[14px] leading-[1.3] font-sans text-[rgba(255,255,255,0.4)]">
              Be the first to organize a town hall for this issue!
            </p>
          </div>
        )}
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 px-3 pb-4">
        <Link
          href={`/demo/cities/mobile/problems/${problemId}/meetups/add`}
