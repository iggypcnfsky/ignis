...
export default function BusinessShareProblemPage() {
	...
	useEffect(() => {
		...
		if (typeof window !== "undefined") {
			setShareUrl(`${window.location.origin}/demo/business/mobile/problems/${problem.id}`);
		}
	}, [problem.id]);
	...
	const socialPostText = useMemo(() => {
		return `Help improve our workplace: ${problem.title}. See details and join the effort: ${shareUrl}`.trim();
	...
	return (
		<div className="relative min-h-dvh w-full bg-[#141414] text-white">
			{/* Shared Header */}
			<SharedHeader mode="back" backHref={`/demo/business/mobile/problems/${problem.id}`} />
			
			{/* Navigation */}
			<div className="relative z-10 px-3 pt-20 pb-2">
				<ProblemPillNav problemId={problem.id} active="share" context="business" />
			</div>
				...
				<div className="grid grid-cols-1 gap-3 lg:grid-cols-12 lg:gap-3">
					...
					>
						<div className="flex items-center gap-2">
							...
							<span className="font-display text-[16px] leading-5">Share link</span>
						</div>
						<div className="mt-4 flex items-center gap-2">
							<div className="flex-1 h-12 rounded-[14px] bg-[#1A1A1A] px-4 flex items-center text-[14px] overflow-hidden">
								...
								<span className="truncate select-all">{shareUrl || `/demo/business/mobile/problems/${problem.id}`}</span>
							</div>
							<button
								type="button"
								onClick={() => copyToClipboard(shareUrl || `/demo/business/mobile/problems/${problem.id}`, "link")}
								className="h-12 px-4 rounded-full bg-[#FF8400] text-white inline-flex items-center gap-2 hover:bg-[#FF8400]/90"
								aria-label="Copy link"
							>
								{copiedLink ? <Check size={16} /> : <Copy size={16} />}
								<span className="text-[14px]">{copiedLink ? "Copied" : "Copy"}</span>
							</button>
						</div>
						<div className="mt-3">
							<button
							...
							>
								Or share via native share sheet
							</button>
						</div>
					</section>
					...
					>
						<div className="flex items-center gap-2">
							...
							<span className="font-display text-[16px] leading-5">QR code</span>
						</div>
						<div className="mt-4 aspect-square bg-[#1A1A1A] rounded-[20px] flex items-center justify-center">
							<div className="text-center text-white/60">
								...
								<p className="text-sm">QR generation</p>
					...
					>
						<div className="flex items-center gap-2">
							...
							<span className="font-display text-[16px] leading-5">A4 poster</span>
						</div>
						<div className="mt-4 rounded-[20px] bg-[#1A1A1A] p-4 min-h-[140px] flex items-center justify-between gap-4">
							<div>
								<p className="text-[14px] text-white/90">Poster with QR and description</p>
								<p className="text-[12px] text-white/50 mt-1">Optimized for printing. Exports as PDF.</p>
							</div>
							<button
					...
					>
						<div className="flex items-center gap-2">
							<Share2 size={18} className="text-white/60" />
							<span className="font-display text-[16px] leading-5">Social post</span>
						</div>
						<div className="mt-4">
							...
							<div className="mt-3 flex items-center justify-end">
								...
								>
									...
									<span className="text-[14px]">{copiedPost ? "Copied" : "Copy text"}</span>
								</button>
							</div>
						</div>
					</section>
