...
export default function BusinessProblemIdeasPage() {
	...
	return (
		<div className="relative min-h-dvh w-full bg-[#141414] text-white">
			...
			<div className="relative z-10 px-3 pt-20 pb-2">
				<ProblemPillNav problemId={problem.id} active="ideas" context="business" />
			</div>
		...
		<main className="px-3 pb-16">
				...
				<div className="space-y-4">
					{problemIdeas.map((idea, index) => (
						...
						>
							<IdeaCard
								className=""
								href={`/demo/business/mobile/problems/${problem.id}/ideas/${idea.id}`}
				...
				<div className="sticky bottom-0 left-0 right-0 pb-4 z-5">
					<button
						type="button"
						onClick={() => router.push(`/ideas/add?problemId=${problem.id}&context=business`)}
