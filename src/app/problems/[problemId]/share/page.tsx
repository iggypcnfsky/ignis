...
export default function ShareProblemPage() {
	...
	useEffect(() => {
		...
		if (typeof window !== "undefined") {
			setShareUrl(`${window.location.origin}/problems/${problem.id}`);
		}
	}, [problem.id]);
	...
	return (
		<div className="relative min-h-dvh w-full bg-[#141414] text-white">
			...
			<div className="relative z-10 px-3 pt-20 pb-2">
				<ProblemPillNav problemId={problem.id} active="share" />
			</div>
					...
					>
						...
						<div className="mt-4 flex items-center gap-2">
							<div className="flex-1 h-12 rounded-[14px] bg-[#1A1A1A] px-4 flex items-center text-[14px] overflow-hidden">
								...
								<span className="truncate select-all">{shareUrl || `/problems/${problem.id}`}</span>
							</div>
							<button
								type="button"
								onClick={() => copyToClipboard(shareUrl || `/problems/${problem.id}`, "link")}
